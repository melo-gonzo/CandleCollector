...
void CandleData::complete() { completed = true; }
...
CandleBuffer::CandleBuffer(size_t maxSize, unsigned int candleDurationSec)
    : maxSize(maxSize), candleDuration(candleDurationSec) {}
...
void CandleBuffer::update(float price) {
  ...
  if (candles.empty() || shouldCreateNewCandle(now)) {
    ...
    time_t startTime = candles.empty() ? alignToInterval(now, candleDuration)
                                       : candles.back()->getEndTime();

    candles.push_back(
        std::unique_ptr<CandleData>(new CandleData(price, startTime)));
